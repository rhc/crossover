...
describe 'Crossover::Client' do
  ...
  it 'can send data to a remote server listening on a given port' do
    ...
    out = capture_io { Crossover::Client.post('Hello, World!', '127.0.0.1', 60000) }.join
  ...
  it 'generates an error message  when there is no server listening' do
    out = capture_io { Crossover::Client.post('Some Info', '127.0.0.1', 54321) }.join
